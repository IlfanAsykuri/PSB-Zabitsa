...
<body class="bg-brown min-h-screen flex items-center justify-center relative overflow-hidden font-sans">
    ...
    <div class="absolute top-[-10%] right-[-10%] w-96 h-96 bg-gold opacity-10 rounded-full blur-3xl mix-blend-overlay"></div>
    <div class="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-gold opacity-10 rounded-full blur-3xl mix-blend-overlay"></div>

    <!-- Centralized Premium Card UI -->
    <div class="relative bg-[#F8F5F0] rounded-2xl shadow-[0_25px_50px_rgba(0,0,0,0.5)] w-full max-w-md p-10 border border-gold/20 z-10">
        
        <!-- Premium gradient header ribbon -->
        <div class="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-gold via-[#D4B483] to-gold rounded-t-2xl"></div>

        <div class="text-center mb-8 mt-2">
            <!-- Icon/Logo Placeholder -->
            <svg class="mx-auto w-12 h-12 text-gold mb-3" fill="none" stroke="currentColor" stroke-width="1.2" viewBox="0 0 24 24">
        ...
        <form method="POST" action="{{ route('backpack.auth.login') }}">
            ...
            <div class="flex items-center mb-8">
                <input type="checkbox" name="remember" id="remember" class="w-4 h-4 text-gold bg-white border-gray-300 rounded focus:ring-gold focus:ring-2 accent-gold" {{ old('remember') ? 'checked' : '' }}>
