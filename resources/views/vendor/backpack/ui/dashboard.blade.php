...
new Chart(eduCtx, {
  ...
  options: {
    ...
    plugins: {
      legend: { 
          position: 'bottom',
          labels: { color: '#4A3B2A', font: { family: "'Inter', sans-serif" } }
      }
    }
  }
});
...
new Chart(statusCtx, {
  ...
  data: {
    ...
    datasets: [{
      label: 'Jumlah Santri',
      data: [{{ $sudahDatang }}, {{ $belumDatang }}, {{ $diverifikasi }}, {{ $prosesVerif }}, {{ $ditolak }}],
      backgroundColor: ['#4A3B2A', '#B59551', '#D4B483', '#8B7355', '#E8E0D5'],
      borderRadius: 6,
      borderWidth: 0
    }]
  ...
  options: {
    ...
    scales: {
      x: { ticks: { color: '#4A3B2A', font: { family: "'Inter', sans-serif" } }, grid: { display:false } },
      y: { beginAtZero: true, ticks: { stepSize: 1, color: '#4A3B2A' }, grid: { color: '#f0f0f0' } }
    }
  }
});
</script>
